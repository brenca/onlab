...
class LogoCore extends Language {
  constructor(handler) {
    ...
    // this.parser.addDisambiguator('Parameters', node => {
    //   console.log('Parameters')
    //   return false
    // })
    
    this.parser.addDisambiguator('Program', stackNode => {
      let customs = {}
      
      const count = (child, name) => {
        let count = 0
        let c = child
        while (c.rule.name === name) {
          count ++
          c = c.children[0]
        }
        return count - 1
      ...
      const handleTo = (c) => {
        ...
        c.children.slice(1).some(ch => {
          if (ch.rule.name === 'Expression') {
            return ch.children.some(chch => {
              if (chch.rule.name === 'To') {
                found = chch
                return true
              }
              return false
        ...
        if (found) {
          let name = new Buffer(found.children[1].rule.value).toString('base64')
          if (found.children.length === 5 && found.children[2].rule.name === 'Variables') {
            customs[name] = count(found.children[2], 'Variables')
          } else {
            customs[name] = 0
          }
        }
      ...
      const handleUserDefined = (c) => {
        let found = null
        
        c.children.slice(1).some(ch => {
          if (ch.rule.name === 'Expression') {
            return ch.children.some(chch => {
              if (chch.rule.name === 'UserDefined') {
                found = chch
                return true
              }
              return false
        ...
        if (found) {
          let name = new Buffer(found.children[0].rule.value).toString('base64')
          if (customs[name]) {
            return count(found.children[1], 'Parameters') === customs[name]
          }
        } else {
          return true
        }
      ...
      let c = stackNode.node
      while (c.children[0].rule.name === stackNode.node.rule.name) {
        handleTo(c)
        c = c.children[0]
      }
      if (c.parent) {
        let firstChild = c.parent.children[1].children[0].rule
        if (firstChild instanceof Lexer.Token && firstChild.class.name === 'var')
          return true
      }
      
      c = stackNode.node
      while (c.children[0].rule.name === stackNode.node.rule.name) {
        if (!handleUserDefined(c))
          return true
        c = c.children[0]
      }
      
      // let c = stackNode.node.children[0]
      // while (c.children[0].rule.name === stackNode.node.rule.name) {
      //   let found = c.children.slice(1).find(ch => {
      //     if (ch.rule.name === 'Expression') {
      //       return ch.children.find(chch => {
      //         return chch.rule.name === 'To'
      //       })
      //     }
      //     return false
      //   })
      //   
      //   if (found) {
      //     console.log(found.children.length === 5);
      //     // if (found.children.length === 5 && found.children[2].rule.name === 'Variables') {
      //     //   customs[found.children[1].rule.value] = count(found.children[2], 'Variables')
      //     // } else {
      //     //   customs[found.children[1].rule.value] = 0
      //     // }
      //   }
      //   // console.log(customs);
      //   c = c.children[0]
  ...
  async execute(code, noreset = false, noexecuteflag = false) {
    if (this._executing && !noreset) {
      ...
      let root
      try {
        let results = this.buildAST(code)
        console.log(results)
        if (results.length > 1) {
          console.log('Multiple parses :O')
        }
        if (results.length >= 1) root = results[0][0]
      } catch (e) {
