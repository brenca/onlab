...
BRNGLR.Action = class BRNGLRAction extends Serializable {
  constructor(input) {
    super()
    this.input = input
  }
...
Serializable.registerSubclass(BRNGLR.Action)

// accept action, marks success
BRNGLR.Accept = class BRNGLRAccept extends BRNGLR.Action {
  constructor(input) {
    super(input)
  }
...
Serializable.registerSubclass(BRNGLR.Accept)

// reduce action
BRNGLR.Reduce = class BRNGLRReduce extends BRNGLR.Action {
...
Serializable.registerSubclass(BRNGLR.Reduce)

// shift action
BRNGLR.Shift = class BRNGLRShift extends BRNGLR.Action {
...
Serializable.registerSubclass(BRNGLR.Shift)

// goto element of the action/goto table
BRNGLR.Goto = class BRNGLRGoto extends BRNGLR.Action {
...
Serializable.registerSubclass(BRNGLR.Goto)
  ...
  const findFollows = (egitem) => {
    ...
    return fast.reduce(getRHSEGRulesForEGItem(egitem), (changed, egrule) => {
      const getRHSFollows = () => {
        ...
        const RHSIndex = RHS.indexOfItem(egitem)
