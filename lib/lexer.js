    ...
    while (str !== undefined && str.length > 0) {
      ...
      this._classes.some(c => {
        if ((!this._state.strict && c.state === Lexer.AnyState)
          ...
          let m = c.match(str)
          if (m.consumed !== false) {
            tokens.push(new Lexer.Token(skipped, m.consumed, c))
            str = m.rest
...
Lexer._StateTransitionBase = class {
  constructor(from, to) {
    this._from = from
    this._to = to
  }
...
Lexer.StateTransition = class extends Lexer._StateTransitionBase {
...
Lexer.GroupStateTransition = class extends Lexer._StateTransitionBase {
