...
// for (var i = 0; i < 5; i++) {
//   X.push(new SLArray())
//   X.get(i).push(new Test())
// }
// console.log(X);
// X.get(0).reset([new Test(), new Test()])
// console.log(X);
...
const gatherLookback = (item, set, n = 0) => {
  ...
  if (!input) {
    return new SLSet(set._transitions.byInput(item.rule))
  }
...
const findIncludes = (item, set) => {
  ...
  return traverse(set, 0).reduce((acc, entry) => {
    return acc.union(new SLSet(entry.set._transitions.byInput(entry.rule)))
  }, new SLSet())
}
...
LALR1.StateTransition = class LALR1StateTransition extends Serializable {
  ...
  get DR() {
    if (!this._dr) {
      this._dr = new SLSet()
      this.to._transitions.terminal.forEach(transition => {
  ...
  get READS() {
    if (!this._reads) {
      ...
      this.to._transitions.nonterminal.forEach(transition => {
...
LALR1.ItemSetArray = class LALR1ItemSetArray extends SLArray {
  ...
  expand() {
    for (let i = 0; i < this.length; i++) {
      const { items, inputs } = this.get(i).advanceDot()
      ...
      inputs.forEach(input => {
        ...
        const transition = new LALR1.StateTransition(input, this.get(i), set)
        this.get(i)._transitions.add(transition)
    ...
    const actions = () => {
      ...
      this.forEach(set => {
        set.actions = set._transitions.terminal.reduce(
          (result, entry) => fast.concat(
            result, new LALR1.Shift(entry.input, entry.to)),
          []
        )

        set.gotos = set._transitions.nonterminal.reduce(
          (result, entry) => fast.concat(
  ...
  get nonterminalTransitions() {
    if (!this._nonterminalTransitions) {
      this._nonterminalTransitions = this.reduce(
        (acc, set) => acc.union(set._transitions.nonterminal),
        new SLSet()
      )
    }
...
LALR1.TransitionTable = class LALR1TransitionTable extends Serializable {
  ...
  byInput(input) {
    if (input.isTerminalRule()) {
      return (this.terminalIndex[input.SID] || new SLSet()).map(
        index => this.terminal.get(index)
      )
    } else {
      return (this.nonterminalIndex[input.SID] || new SLSet()).map(
        index => this.nonterminal.get(index)
      )
    }
  }
}
