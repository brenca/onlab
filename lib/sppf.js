...
const flattenArcs = (arcs) => {
  return fast.reduce(fast.map(arcs, arc => {
...
SPPF.Node = class SPPFNode extends Serializable {
  ...
  flattenGeneratedRules(seen = {}) {
    if (seen[this.SID]) return
    seen[this.SID] = true

    this.arcs = fast.reduce(fast.map(this.arcs, arc => {
      arc.flattenGeneratedRules(seen)

      if (arc.action && arc.action.rule instanceof BNF.GeneratedRule) {
        return arc.arcs
      }
      return arc
    }), (a, x) => fast.concat(a, x), [])
...
SPPF.Forest = class SPPFForest extends Serializable {
  ...
  constructor(root, ambiguous = true) {
    ...
    // this.root.flattenGeneratedRules()
    if (this.transform()) {
  ...
  transform(transforms = [], dropDefault = false) {
    ...
    const transformList = skip ? transforms : [
      ...
      , (node) => {
        ...
        fast.forEach(node.arcs, arc => {
          paths[arc.SID] = fast.map(arc.arcs, arc =>
            fast.reduce(
        ...
        node.arcs = fast.filter(node.arcs, arc => keepMe.has(arc.SID))
      }
    ].concat(transforms)
