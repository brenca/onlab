...
class Test extends Language {
  ...
  execute(code) {
    ...
    // const ambiguity = (node) => {
    //   const a = node.arcs.filter(
    //     arc => arc instanceof Parser._SPPFPackedNode).length
    //
    //   return node.arcs.reduce((acc, x) => {
    //     return acc + ambiguity(x)
    //   }, 1) * (a > 0 ? a : 1)
    // }
    // console.log(ambiguity(ast))
