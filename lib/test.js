...
class Test extends Language {
  ...
  execute(code) {
    try {
      ...
      const log = fs.createWriteStream('test.dot', { flags: 'w' })
      ...
      nodes.forEach(node => {
        ...
        const shapetext = shape(node) ? `shape="${shape(node)}"` : ''

        console.log(`"${node.uuid}" [label="${node.toString()}" ${node.dotStyle()}]`);
      })
      console.log()
      nodes.forEach(node => {
        node.arcs.forEach(arc => {
          console.log(`"${node.uuid}" -> "${arc.uuid}"`);
        })
      })

      console.log(`{rank = same; ${nodes.filter(node => {
        return node.item && node.item.value !== undefined
      ...
      // const exec = (node) => {
      //   const args = node.arcs.map(exec)
      //
      //   switch (node.item.name) {
      //     case 'E': {
      //       switch (node.subruleIndex) {
      //         case 0: {
      //           switch (args[1]) {
      //             case '+': {
      ...
      //
      // trees.forEach(tree => {
      //   console.log(toString(tree), exec(tree))
      // })
    } catch (e) {
