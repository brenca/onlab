...
class Parser {
    ...
    mergeEGRules().fastForEach(mr => {
      mr.follows.fastForEach(f => {
        if (mr.finalSet !== undefined) {
          ...
          let testAction = a => (a.input.equals(f) && a.equals(reduce))
            || a instanceof Parser._Accept
  ...
  parse(code) {  
    let determineWhatToDo = () => {
      ...
      let actions = stack._actions.fastFilter(a => {
        if (a.input.value !== undefined) {
          let equals = !a.input.value.split('').fastSome((c, i) => {
            return c !== this._state.stack.getInput(i).value
          })
          
          if (equals && a.input.value.length > 0) return a
        } else if (a.input.tokenClass !== undefined) {
          if (a.input.tokenClass === this._state.stack.getInput().tokenClass)
            return a
          if (a.input.tokenClass === this._state.stack.getInput().class)
            return a
        }
      })
      ...
      if (actions.length === 0) {
        if (this._state.stack.getInput().tokenClass instanceof Lexer.EOFTokenClass) {
          throw new Parser.SyntaxError(
            'Unexpected end of input', 
            this._state.stack.getInput().position, 
            this._state.stack.getInput()
          )
        }
        
        throw new Parser.SyntaxError(
          'Unexpected input "' + this._state.stack.getInput().value + '"', 
          this._state.stack.getInput().position, 
          this._state.stack.getInput()
        )
      }
    ...
    try {
      while(true) {
        ...
        // skip empty lines
        while (this._state.stack.getInput().class instanceof Lexer.EOLTokenClass) {
          this._state.stack.shiftInput()
        }
      }
    } catch (e) {
    ...
    let nodes = []
    while (this._state.stack !== this._state.root) {
      nodes.unshift(this._state.stack.node)
      this._state.stack = this._state.stack.parent
    }
    nodes.fastForEach(o => {
      o.reduce()
    })
...
Parser._StateTree = class {
  ...
  pop(num) {
    let nodes = []
    let hadMultipleChildren = false
    
    for (let i = 0; i < num; i++) {
      if (this.stack.parent.children.length > 1) hadMultipleChildren = true
      
      if (!hadMultipleChildren) {
        this.stack.parent.removeChild(this.stack)
      }
      if (this.stack.node) {
        nodes.unshift(this.stack.node)
      }
...
Parser._Reduce = class extends Parser._Action {
  ...
  execute(parser) {
    ...
    let inputIndex = parser._state.stack.inputIndex
    let nodes = parser._state.pop(num)
    ...
    if (goto === undefined) {
      throw new Parser.SyntaxError('Unexpected token "' + 
        (parser._state.stack.getInput().name || parser._state.stack.getInput().value) + '"',
        parser._state.stack.getInput().position
      )
    }
    return goto.execute(parser, new Parser.Node(this.rule, nodes, this.i), inputIndex)
  }
...
Parser._Shift = class extends Parser._Action {
  ...
  execute(parser) {
    let stackTop = parser._state.push(this._itemSet)
    if (this.input.value === undefined || this.input.value.length === 1) {
      // parser._state.index++
      stackTop.node = new Parser.Node(parser._state.stack.shiftInput(), [])
    } else {
      let val = ''
      let position = parser._state.stack.getInput().position
      this.input.value.split('').fastForEach(c => {
        // parser._state.index++
        val += parser._state.stack.shiftInput().value
      })
...
Parser._Goto = class extends Parser._Action {
  ...
  execute(parser, node, inputIndex) {
    let newTop = parser._state.push(this.to)
