...
class Parser {
  ...
  parse(code) {  
    let determineWhatToDo = (stack) => {
      let actions = stack._actions.fastFilter(a => {
        if (a.input.value !== undefined) {
          ...
          if (a.input.tokenClass === stack.getInput().tokenClass)
            return a
          if (a.input.tokenClass === stack.getInput().class)
            return a
        }
      })
      
      if (actions.length === 0) {
        actions = stack._actions.fastFilter(a => {
          if (a.input.value !== undefined) {
      ...
      return actions.sort((a, b) => {
        if (a instanceof Parser._Shift) return -1
        if (b instanceof Parser._Shift) return 1
        return 0
      })
    }
    ...
    while(stoppedAt === null) {
      ...
      // let newLeavesReal = getLeaves(this._state.root)
      // console.log(newLeavesReal, newLeaves);
    ...
    // console.log(getLeaves(this._state.root));
    
    let nodes = []
    while (stoppedAt !== this._state.root) {
      nodes.unshift(stoppedAt.node)
      stoppedAt = stoppedAt.parent
    }
    nodes.fastForEach(o => {
      o.reduce()
    })
...
Parser._Reduce = class extends Parser._Action {
  ...
  execute(stackTop) {
    ...
    return goto.execute(stack, new Parser.Node(this.rule, nodes, this.i), inputIndex)
  }
