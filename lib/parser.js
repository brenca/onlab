...
class Parser {
  ...
  _getEpsilonSPPFIndexForRHS(rul, m) {
    const rhs = rul.rule.subrules[rul.i]
          ...
          while (R.length > 0) { // Reducer
            ...
            paths.fastForEach(path => {
              ...
              if (action.m === 0) {
                ...
                z = N.fastFind(n => {
                  return n.action.rule === action.rule
                    && n.left === c
                })
                if (!z) {
                  z = new Parser._SPPFSymbolNode(action, c, inputIndex)
                  N.push(z)
                }
              }
  ...
  _copyArcs(decisions, map, other) {
    ...
    other.arcs = this.arcs.fastMap((arc, i) => {
      if (!map[arc.hash]) {
        ...
        // if (decisions[arc.hash] !== undefined) {
        //   let dec = Object.assign({}, decisions)
        //   dec[arc.hash] = (decisions[arc.hash] + 1) % arc.arcs.length
        //
        //   return arc.clone(dec, map)
        // }
        return map[arc.hash]
      }
    })
