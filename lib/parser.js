...
class Parser {
  ...
  fromBNF(bnf) {
    ...
    if (nullables.length > 0) {
      ...
      realRules.forEach(rule => {
        ...
        rule.subrules.forEach((rules, index) => {
        ...
        if (nullableWholeSubrules.length > 0) {
          ...
          if (nullableWholeSubrules.length === 1) {
            nullableWholeSubrules[0].rules.forEach(r => {
              if (this._epsilonSPPFMap[r.hash()]) {
            ...
            nullableWholeSubrules.forEach(subrule => {
    ...
    items.fastForEach(item => {
      findFromTo(item.set, item.item.rule).fastForEach(ft => {
        ...
        item.item.rule.subrules[item.item.i].fastForEach(sr => {
          let nrightHandSides = []
          rightHandSides.fastForEach(rightHandSide => {
    ...
    let first = (egitem) => {
      ...
      getLHSEGRulesForEGItem(egitem).fastForEach(egrule => {
        if (egrule.rightHandSide[0].rule.isTerminalRule()) {
          const isInFirsts = _isInArray(egrule.rightHandSide[0].rule,
          ...
          if(!egrule.rightHandSide.fastSome(r => {
            if (r.rule instanceof Parser.BNFRule) {
              let hasEpsilon = false

              r.firsts.fastForEach(f => {
                if (!f.isEpsilonRule()) {
                  if (!_isInArray(f, egrule.leftHandSide.firsts)) {
                    changed++
              ...
              if (!_isInArray(r.rule, egrule.leftHandSide.firsts)) {
                changed++
            ...
            if (!_isInArray(epsilon, egrule.leftHandSide.firsts)) {
              changed++
  ...
  _calculateFollows() {
    let follow = (egitem) => {
      ...
      getRHSEGRulesForEGItem(egitem).fastForEach(egrule => {
        let index = egrule.rightHandSide.indexOf(egitem)
        if (index === egrule.rightHandSide.length - 1) {

          egrule.leftHandSide.follows.fastForEach(f => {
  ...
  _calculateActionsAndGotos() {
    ...
    this._egrules.fastForEach(mr => {
      mr.leftHandSide.follows.fastForEach(follow => {
        if (mr.getFinalSet() !== undefined) {
          let reduceRules = mr.getFinalSet().items.fastFilter(item => {
          ...
          if (reduceRules.length !== 0) {
            reduceRules.fastForEach(reduceRule => {
              ...
              let reduce = new Parser._Reduce(follow, reduceRule, m, f)
  ...
  parse(code) {
    ...
    if (code.length === 0) {
      ...
      const completeReduction = (node, action, c, inputIndex, N) => {
        const pl = node.itemSet._gotos.fastFind(goto =>
          goto.input.equals(action.rule))
        let z = (() => {
          let ret = N.fastFind(n =>
            n.action.rule === action.rule && n.left === c)
          if (!ret) {
            ret = new Parser._SPPFSymbolNode(action, c, inputIndex)
            N.push(ret)
          }
          return ret
        })()
        ...
        for (; inputIndex < length && U[inputIndex].length > 0; inputIndex++) {
          ...
          while (R.length > 0) { // Reducer
            ...
            if (action.m === 0) {
              ...
              const c = U.fastFindIndex(Uc => Uc.indexOf(node) >= 0)
              completeReduction(node, action, c, inputIndex, N)
              const z = N.fastFind(n =>
                n.action.rule === action.rule && n.left === c)
              addChildren(z, [y], f, action)
            } else if (action.m === 2) {
              node.arcs.fastForEach((arc, i) => {
                const c = U.fastFindIndex(Uc => Uc.indexOf(arc) >= 0)
              ...
              let w = U[inputIndex].fastFind(u => u.itemSet === Xm)
              if (!w) {
...
Parser._SPPFNode = class {
  ...
  packs(alfa) {
    return alfa.length === this.arcs.length &&
      alfa.reduce((a, x, i) => a && x.equals(this.arcs[i]), true)
  }
  ...
  _copyArcs(decisions, map, other) {
    map[other.hash] = other
    other.arcs = this.arcs.fastMap((arc, i) => {
      if (!map[arc.hash]) {
        return arc.clone(decisions, map)
      } else {
        return map[arc.hash]
      }
    })
  ...
  clone(decisions = {}, map = {}) {
    return this._copyArcs(decisions, map, new this.constructor(
      this.j, this.hash))
  }
}
...
Parser._SPPFIntermediateNode = class extends Parser._SPPFNode {
  ...
  toString() {
    const rule = `'${this.action.rule.name}' ::= ${
      this.action.rule.subrules[this.action.i].map((rule, i) => {
