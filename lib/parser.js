...
class Parser {
    ...
    this._itemSets.fastForEach(set => {
      set.getAfterDotSet().fastForEach(ad => {
        ...
        set.createItemsWithDotBefore(ad).fastForEach(idb => {
          getItemSetsForItem(idb).forEach(s => {
            sets.push(s)
          })
        })
        sets = [...new Set(sets)]
      ...
      this._egrules.fastForEach(egr => {
        ...
        if (!mergedRules.fastSome(mr => {
          ...
          mergedRules.push({
            ...
            i: similar[0].i,
            follows: [...new BNFSet([].concat.apply([], similar.map(s => {
    ...
    mergeEGRules().fastForEach(mr => {
      mr.follows.fastForEach(f => {
        if (mr.finalSet !== undefined) {
          ...
          let testAction = a => a.equals(reduce) || a instanceof Parser._Accept
          if (!mr.finalSet._actions.fastSome(testAction))
  ...
  parse(code) {
    let determineWhatToDo = (stack) => {
      ...
      stack._actions.fastForEach(a => {
        if (a.input.value !== undefined) {
          let equals = !a.input.value.split('').fastSome((c, i) => {
            return c !== stack.getInput(i).value
          })
          
          if (equals) categorize(a)
        } else if (a.input.tokenClass !== undefined) {
          if (a.input.tokenClass === stack.getInput().tokenClass ||
              a.input.tokenClass === stack.getInput().class) {
            categorize(a)
          }
        }
      })
    ...
    while(this._state.leaves.fastSome(leaf => !leaf.successfulParse)) {
      let newLeaves = this._state.leaves.fastFilter(leaf => !!leaf.successfulParse)
      
      this._state.leaves.fastForEach(leaf => {
        if (leaf.successfulParse) return
        let actions = determineWhatToDo(leaf)
        if (actions.length === 0) {
          if (this._state.leaves.length === 1) {
            if (leaf.getInput().tokenClass instanceof Lexer.EOFTokenClass) {
              throw new Parser.SyntaxError(
                'Unexpected end of input', 
                leaf.getInput().position, 
                leaf.getInput()
              )
            }
            
            throw new Parser.SyntaxError(
              'Unexpected input "' + leaf.getInput().value + '"', 
              leaf.getInput().position, 
              leaf.getInput()
            )
          } else {
            return leaf.removeFromTree()
          }
        }
        actions.fastForEach(action => {
          try {
            let actionResult = action.execute(leaf)
            if (actionResult) {
              if (this._disambiguate(actionResult)) {
                actionResult.removeFromTree()
              } else {
                // skip empty lines
                while (actionResult.getInput().class instanceof Lexer.EOLTokenClass) {
                  actionResult.shiftInput()
                }
                newLeaves.push(actionResult)
              }
            } else {
              leaf.successfulParse = true
              if (newLeaves.indexOf(leaf) === -1)
                newLeaves.push(leaf)
            }
          } catch (e) {
            return leaf.removeFromTree()
      ...
      if (newLeaves.length > 0) {
        this._state.leaves = newLeaves
      } else {
        let leaf = this._state.leaves[0]
        throw new Parser.SyntaxError(
          'Unexpected input "' + leaf.getInput().value + '"', 
          leaf.getInput().position, 
          leaf.getInput()
    ...
    this._state.leaves.fastForEach(leaf => {
      let nodes = []
      while (leaf !== this._state.root) {
        nodes.unshift(leaf.node)
        leaf = leaf.parent
      }
      nodes.fastForEach(o => {
        o.reduce()
...
Parser._StateTree = class {
  constructor(parser, input, rootSet) {    
    ...
    EOF.position = input.length > 0 ? input[input.length - 1].position : {}
    EOF.position.char += input.length > 0 ? input[input.length - 1].value.length : 0
...
Parser._StateTreeNode = class {
  constructor(itemSet, parent, inputIndex) {
    this.itemSet = itemSet
    this.parent = parent
    if (this.parent) {
      this.tree = this.parent.tree
    }
    this.inputIndex = parent ? parent.inputIndex : 0
    this.children = []
    this.node = undefined
  }
  ...
  getInput(i = 0) {
    return this.tree.input[this.inputIndex + i]
  }
  
  shiftInput() {
    return this.tree.input[this.inputIndex++]
  }
  ...
  removeFromTree() {
    let node = this
    while (node.parent.children.length === 1 && node.parent !== this.tree.root) {
  ...
  push(itemSet) {
    let newTop = new Parser._StateTreeNode(itemSet, this)
    this.addChild(newTop)
    
    return newTop
  }
  
  flush() {
    if (this.successfulParse) return
    let hadMultipleChildren = this.children.length > 0
    let stack = this

    while (!hadMultipleChildren && stack !== this.tree.root) {
      if (stack.parent.children.length > 1) hadMultipleChildren = true
...
Parser._Accept = class _Accept extends Parser._Action {
  ...
  equals(other) {
    return super.equals(other) && other instanceof Parser._Accept
  }
}
...
Parser._Reduce = class _Reduce extends Parser._Action {
  ...
  execute(stackTop) {
    ...
    let goto = stack._gotos.fastFind(
      g => {
        return g.input.equals(this.rule)
      }
    )
  ...
  equals(other) {
    return super.equals(other) && other instanceof Parser._Reduce
...
Parser._Shift = class _Shift extends Parser._Action {
  ...
  execute(stack) {
    let stackTop = stack.push(this._itemSet)
    if (this.input.value === undefined || this.input.value.length === 1) {
      // parser._state.index++
      stackTop.node = new Parser.Node(stackTop.shiftInput(), [])
    } else {
      let val = ''
      let position = stackTop.getInput().position
      this.input.value.split('').fastForEach(c => {
        // parser._state.index++
        val += stackTop.shiftInput().value
      })
      stackTop.node = new Parser.Node({
        skipped: 0,
        value: val,
        position,
        class: {
  ...
  equals(other) {
    return super.equals(other) && other instanceof Parser._Shift
...
Parser._LR0ItemSet = class {
  ...
  isIncluded(rule) {
    return this.items.find(i => { 
      return i.rule.equals(rule) && i.dot === 0 
    }) !== undefined
  }
  ...
  getAfterDotSet() {
    ...
    return [...new BNFSet(afterdot)].sort((a, b) => {
...
Parser.BNFTerminal = class extends Parser._RuleTerminalBase {
  ...
  equals(other) {
    return other instanceof Parser.BNFTerminal && (this.value.localeCompare(other.value) === 0)
  }
