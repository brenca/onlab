  ...
  _calculateFollows() {
    let follow = (egitem) => {
      let getRHSEGRulesForEGItem = (egitem) => {
        let matching = []
        for (let i = 0; i < this._egrules.length; i++) {
          let r = this._egrules[i]
          for (let j = 0; j < r.rhs.length; j++) {
  ...
  parse(code) {  
    let determineWhatToDo = () => {
      ...
      if (this._state.input.length === 0) {
        this._state.input.push(this._findRule('Token-EOF'))
      }
    ...
    EOF.position = code[code.length - 1].position
    EOF.position.char += code[code.length - 1].value.length
    this._state = {
      input: code.concat([EOF]),
      index: 0,
      nodes: [],
...
Parser._Reduce = class extends Parser._Action {
  ...
  execute(parser) {
    ...
    let goto = parser._state.stack[parser._state.stack.length - 1]._gotos.find(
      g => {
